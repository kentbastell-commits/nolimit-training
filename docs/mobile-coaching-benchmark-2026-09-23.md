...
Implementation update, 23 September: releases A–C and the web portion of D are implemented. [Release E / phase 5](mobile-coaching-release-e-2026-09-23.md) adds cross-device builder drafts, reviewed weekly progression and measured assignment/reply routes. It also aligns mini-program dates with China time; the development upload still requires trial selection and WeChat review. Physical-device keyboard/background checks remain open. Nutrition tracking, community feeds, marketplace expansion, AI-generated programs and a new native coach app remain lower priorities for the current online-coaching operation.

[Private calendar drafts](calendar-drafts-2026-09-23.md) extend this workflow: save a plan onto the coach's calendar, continue editing privately, then review and publish selected sessions. These are separate from builder recovery drafts. Visibility is enforced by the server for both the web athlete view and the existing mini program.

[Session editing and calendar planning](session-planning-2026-09-23.md) adds the five agreed follow-ups: consistent bulk targets, Undo and saved-session history, detailed publish previews, private revisions to live workouts, and multi-session Move/Copy with reviewed dates.
