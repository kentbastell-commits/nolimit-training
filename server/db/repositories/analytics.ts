...
export async function getAnalytics(): Promise<AnalyticsResult> {
  // Derived dashboard over two large scans; a short TTL keeps the Dashboard
  // snappy without needing to invalidate on every workout write.
  const cached = getCached<AnalyticsResult>("analytics");
