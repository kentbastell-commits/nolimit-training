// Feishu implementation of the form templates domain. Logic moved verbatim
// from the pre-repository api/formTemplates.ts handler; shared client helpers
// replace the handler's inline token/pagination/CRUD fetches where they are
// drop-in equivalents. Field parsing/aliasing, fallbacks and error bodies are
// unchanged.
import {
  getTenantToken,
  appToken,
...
type Tables =
  | { ok: true; templates: string; questions: string }
  | { ok: false; error: FormTemplatesOpResult };
...
function tableIds(): Tables {
  ...
  if (!templates) {
    return {
      ok: false,
      error: { status: 500, body: { error: "Missing FEISHU_FORM_TEMPLATES_TABLE_ID" } },
    };
  }
  ...
  if (!questions) {
    return {
      ok: false,
      error: { status: 500, body: { error: "Missing FEISHU_FORM_QUESTIONS_TABLE_ID" } },
    };
  }

  return { ok: true, templates, questions };
}

const questionFieldSpecs = (
  formId: string,
  index: number,
  question: any
...
export async function deleteFormTemplate(
  ...
  const tables = tableIds();
  if (!tables.ok) return tables.error;
  const { recordId, formId } = input;

  const questionRecords = await listRecords(tables.questions);
  ...
  for (const question of childQuestions) {
    ...
    if (data.code !== 0) {
      return {
        status: 500,
        body: { error: "Could not delete form question", larkResponse: data },
      };
    }
  ...
  if (templateData.code !== 0) {
    return {
      status: 500,
      body: { error: "Could not delete form template", larkResponse: templateData },
    };
  }
...
export async function updateFormTemplate(
  ...
  const [formTemplateFieldNames, formQuestionFieldNames] = await Promise.all([
    ...
    getTableFieldNames(tables.questions),
  ]);

  const { recordId, formId, name, type, description, status, createdBy, questions } =
    input;
  ...
  const existingQuestions = await listRecords(tables.questions);
  ...
  for (const question of childQuestions) {
    ...
    if (data.code !== 0) {
      return {
        status: 500,
        body: { error: "Could not replace form questions", larkResponse: data },
      };
    }
  }
  ...
  for (const [index, question] of questionItems.entries()) {
    const { fields: questionFields, missingRequired: missingQuestionFields } =
      buildFields(formQuestionFieldNames, questionFieldSpecs(String(formId), index, question));
    ...
    const questionData = await createRecord(tables.questions, questionFields);
...
export async function createFormTemplate(
  ...
  const { name, type, description, status, createdBy, questions } = input;
  ...
  for (const [index, question] of questionItems.entries()) {
    const { fields: questionFields, missingRequired: missingQuestionFields } =
      buildFields(formQuestionFieldNames, questionFieldSpecs(formId, index, question));
