...
const baseProps = {
  ...
  portalPaymentPending: false,
  retryPendingPayment: vi.fn(),
...
describe("PortalWelcome", () => {
  ...
  it("shows a truthful waiting state until payment is verified", () => {
    const retryPendingPayment = vi.fn();
    render(
      <PortalWelcome
        {...baseProps}
        portalPaymentPending={true}
        retryPendingPayment={retryPendingPayment}
      />
    );

    expect(screen.getByText("Payment verification in progress")).toBeInTheDocument();
    expect(screen.queryByText("You're all set, Wei!")).not.toBeInTheDocument();
    fireEvent.click(screen.getByText("Check payment status again"));
    expect(retryPendingPayment).toHaveBeenCalledTimes(1);
  });
